...
module.exports = Field.create({
	...
	getImageURL () {
		if (!this.hasLocal() && this.hasExisting()) {
			return this.props.value.hrefPrefix + this.props.value.filename;
		}
	},
	...
	renderImagePreview () {
		...
		var href = this.getImageURL();

		if (href) {
			body = <a className="img-thumbnail" href={href} onClick={this.openLightbox.bind(this, 0)} target="__blank">{body}</a>;
		} else {
	...
	renderImagePreviewThumbnail () {
		...
		return <img key={this.props.path + '_preview_thumbnail'} className="img-load" style={ { height: '90' } } src={href} />;
	},
		...
		if (!this.hasLocal() && !this.state.removeExisting) {
			values = (
				<div className="image-values">
					<FormInput noedit>{this.getImageURL()}</FormInput>
					{/*
	...
	renderAlert () {
		if (this.hasLocal()) {
			...
			return (
				<FormInput noedit>Image selected from Local</FormInput>
			);
		} else if (this.state.removeExisting) {
			return (
