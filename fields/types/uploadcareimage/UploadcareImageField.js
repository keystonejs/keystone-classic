...
var SUPPORTED_TYPES = ['image/gif', 'image/png', 'image/jpeg', 'image/bmp', 'image/x-tiff'];

module.exports = Field.create({
	
	displayName: 'UploadcareImageField',
	...
	getImageSource: function() {
		if (this.hasLocal()) {
			...
			return this.props.value.original_file_url.replace('http://www.uploadcare.com', 'https://uploadcare.com');
		} else {
			return null;
		}
	},
		...
		if (window.FileReader) {
			...
			_.each(files, function (f) {
				if (!_.contains(SUPPORTED_TYPES, f.type)) {
					self.removeImage();
					alert('Unsupported file type. Supported formats are: GIF, PNG, JPG, BMP, TIFF');
					return false;
				}
	...
	hasExisting: function() {
		return !!this.props.value.original_file_url;
	},
		...
		if (!this.hasLocal() && !this.state.removeExisting) {
			values = (
				<div className='image-values'>
					<div className='field-value'>{this.props.value.original_file_url}</div>
	...
	renderImageDimensions: function() {
		return <div className='field-value'>{this.props.value.image_info.width} x {this.props.value.image_info.height}</div>;
	},
	 ...
	 *  - On a uploadcare file, output a "from uploadcare" message.
	...
	renderAlert: function() {
		...
		} else if (this.state.origin === 'uploadcare') {
			return (
				<div className='select-queued pull-left'>
					<div className='alert alert-success'>Image selected from Uploadcare</div>
				</div>
			);
		} else if (this.state.removeExisting) {
			return (
	...
	renderImageSelect: function() {
		var getOptions = function(input, callback) {
			$.get('/keystone/api/uploadcare/autocomplete', {
				dataType: 'json',
				data: {
					q: input
				}
		...
		return (
			<div className='image-select'>
				<Select
					...
					className='ui-select2-uploadcare'
	...
	renderUI: function() {
		...
		return (
			<div className='field field-type-uploadcareimage'>
