...
* The template locales are now passed in as the first, the options for send as the second and the callback as the third argument to `.send`

  ```JS
  new Keystone.Email('templatepath.ext').send(templateLocales, sendOptions, callback);
  ```
