...
var fieldRegistry = require('keystone/lib/fieldRegistry');
...
module.exports = function (file, name) {
	...
	var ready;
	var src;
	function exposeBuiltins (b) {
		var vFiles = [
			new File({
				contents: new Buffer(fieldRegistry.generateJSForComponentType('Filter', function(x) { return x.toLowerCase(); })),
				path: 'builtin-filters',
				base: basedir
			}),
			new File({
				contents: new Buffer(fieldRegistry.generateJSForComponentType('Field', function(x) { return x.toLowerCase(); })),
				path: 'builtin-fields',
				base: basedir
			}),
			new File({
				contents: new Buffer(fieldRegistry.generateJSForComponentType('Column', function(x) { return x.toLowerCase(); })),
				path: 'builtin-columns',
				base: basedir
			}),
		];
		...
		{
		  // See here: https://github.com/substack/node-browserify/issues/1106
		  b.exclude(vFile.path);
		  b.require(vFile, { expose: vFile.path });
		});
	};
	...
	function build () {
		...
		var built = function built (err, buff) {
		...
		fieldRegistry.on('added', function () {
			exposeBuiltins(b);
			b.bundle(built);
		});
	}
