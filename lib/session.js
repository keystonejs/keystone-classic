var prospekt = require('../'),
	utils = require('./utils'),
	User = prospekt.list(prospekt.get('user model'));
...
var signin = exports.signin = function(lookup, req, res, onSuccess, onFail) {
	...
	if (utils.isObject(lookup)) {
		...
		User.model.findOne({ email: lookup.email }).exec(function(err, user) {
			if (user) {
				user.comparePassword(lookup.password, function(err, isMatch) {
					if (!err && isMatch) {
						doSignin(user);
					}
					else {
...
/**
 * Middleware to enable access to Prospekt
 * 
...
exports.prospektAuth = function(req, res, next) {
	
	if (!req.user || !req.user.canAccessProspekt)
		return res.redirect('/prospekt/signin');
	next();
