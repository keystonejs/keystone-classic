...
module.exports = function(keystone){
	...
	sessionExports.signinWithUser = signinWithUser;
	...
	var postHookedSigninWithUser = function(user, req, res, onSuccess, onFail) {
		keystone.callHook(user, 'post:signin', function(err) {
			...
			sessionExports.signinWithUser(user, req, res, onSuccess, onFail);
		});
	};
	...
	sessionExports.signin = function(lookup, req, res, onSuccess, onFail) {
	...
	sessionExports.signout = function(req, res, next) {
	...
	sessionExports.persist = function(req, res, next) {
		var User = keystone.list(keystone.get('user model'));
		if (!req.session) {
		...
		if (keystone.get('cookie signin') && !req.session.userId && req.signedCookies['keystone.uid'] && req.signedCookies['keystone.uid'].indexOf(':') > 0) {
			sessionExports.signin(req.signedCookies['keystone.uid'], req, res, function() {
				next();
			}, function(err) {
				res.clearCookie('keystone.uid');
				req.user = null;
	...
	sessionExports.keystoneAuth = function(req, res, next) {
