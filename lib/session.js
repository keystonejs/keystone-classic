...
var whitelistRE;
var keystoneRE = new RegExp('^/keystone/?$', 'i');
var apiRE = new RegExp('^/keystone/api/');
exports.keystoneAuth = function(req, res, next) {
	if (!req.user || !req.user.canAccessKeystone) {
		if (!checkedAllow) {
			checkedAllow = true;
			var allowed = keystone.get('api whitelist');
			if (allowed) {
				var regexes = allowed.join('|');
				whitelistRE = new RegExp('^/keystone/api/(' + regexes + ')(/|\\?|$)');
			}
		}
		if (whitelistRE && req.method === 'GET' && whitelistRE.test(req.url)) {
			return next();
		}
		if (apiRE.test(req.url)) {
			return res.status(403).send('Not logged in and not whitelisted');
		} else {
			var from = keystoneRE.test(req.url) ? '' : '?from=' + req.url;
