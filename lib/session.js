		...
		User.model.findOne({ email: lookup.email }).exec(function(err, user) {
			if (user) {
				user._.password.compare(lookup.password, function(err, isMatch) {
					...
					} else if (user.temporaryPassword != null && user.temporaryPasswordValidUntil != null && moment().diff(user.temporaryPasswordValidUntil) < 0) {
						user._.temporaryPassword.compare(lookup.password, function(err, isMatch) {
							if (!err && isMatch) {
								exports.signinWithUser(user, req, res, onSuccess);
							} else {
								onFail(err);
...
exports.checkResetPassword = function(lookup, req, res, onSuccess, onFail) {
    if (!lookup) {
    	return onFail(new Error('Keystone.session.sendResetPassword requires an Object as the first argument'));
    }
    var User = keystone.list(keystone.get('user model'));
    if ('string' === typeof lookup.email) {
        ...
        User.model.findOne({ email: lookup.email }).exec(function(err, user) {
            ...
            if (user && user.sendResetPassword) {
        		// console.log('FOUND A USER WITH EMAIL', lookup.email);
                user.sendResetPassword(onSuccess);
            } else {
        		// console.log('NO USER FOUND WITH EMAIL', lookup.email);
                onFail(err);
            }
        });
    } else {
...
exports.sendResetPassword = function(user, callback) {
	...
	var temporaryPassword = crypto.pseudoRandomBytes(8).toString('base64'),
		temporaryPasswordValidUntil = moment().add(1, 'day');

	user.set('temporaryPassword', temporaryPassword);
	user.set('temporaryPasswordValidUntil', temporaryPasswordValidUntil);

	user.save(function(err) {

		if (err) {
			console.log('Error saving changes to user temporaryPassword');
			console.log(err);
		}
