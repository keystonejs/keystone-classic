...
function addValidationError (errors, field, type, detail) {
	if (detail instanceof Error) {
		detail = detail.name !== 'Error' ? detail.name + ': ' + detail.message : detail.message;
	}
	errors[field.path] = {
		type: type,
		error: typeof detail === 'string' ? detail : field.path + ' is ' + type,
...
function updateItem (item, data, options, callback) {
	...
	function doFieldValidation (field, done) {
		// only validate fields we're going to update
		if (field.noedit && !ignoreNoEdit) {
			return done();
		}
		...
		field.validateInput(data, function (valid, detail) {
			if (!valid) {
				addValidationError(validationErrors, field, 'invalid', detail);
				done();
			} else {
					...
					&& (!field.dependsOn || evalDependsOn(field.dependsOn, data)))
				{
					field.validateRequiredInput(item, data, function (valid, detail) {
						if (!valid) {
							addValidationError(validationErrors, field, 'required', detail);
						}
						done();
					});
				} else {
	...
	function doFieldUpdate (field, done) {
		...
		// check we should update the field
		if (field.noedit && !ignoreNoEdit) {
			return done();
		}
