...
List.prototype.getSearchFilters = function(q, add) {
	
	var filters = {};
	
	var wrap = function(val, inv) {
		if (inv) {
			var cond = {};
			cond[val instanceof RegExp ? '$not' : '$ne'] = val;
			return cond;
		} else {
			return val;
		}
	...
	var and = function(path, val, inv) {
		filters[path] = wrap(val, inv);
	}
	
	var or = function(path, val, inv) {
		var cond;
		if (!filters.$and) {
			filters.$and = [];
		}
		...
		for (var i = 0; i < val.length; i++) {
			var _or = {};
			_or[path] = wrap(val[i], inv);
			cond.$or.push(_or);
		}
		filters.$and.push(cond);
	}
	
	if (q && q.length) {
	
		var qr = new RegExp(q, 'i');
		var searchIdField = utils.isValidObjectId(q);
		var searchFields = this.get('searchFields');
		
		if (searchFields == '__name__') {
			
			searchFields = [this.mappings.name];
			var nameField = this.fields[this.mappings.name];
			
			if (nameField && nameField.type == 'name') {
				searchFields = [nameField.paths.first, nameField.paths.last];
			}
			
			// add the autokey as a default search field
			if (this.autokey) {
				searchFields.push(this.autokey.path);
			};
		...
		if (searchFields.length == 1 && !searchIdField) {
			filters[searchFields[0]] = qr;
		} else {
			...
			if (filter.field) {
				switch (filter.field.type) {
					...
					case 'key':
						if (filter.exact) {
							if (filter.value) {
								and(filter.field.path, new RegExp('^' + utils.escapeRegExp(filter.value) + '$', 'i'), filter.inv);
							} else {
								if (filter.inv) {
									and(filter.field.path, { $nin: ['', null] });
								} else {
									or(filter.field.path, ['', null]);
								}
							}
