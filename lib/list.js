...
function List(key, options) {
	...
	Object.defineProperty(this, 'formFields', { get: function() {
		if (formFields)
			return formFields;
		var exceptFields = ['_id', 'id', 'createdOn', 'createdBy', 'changedOn', 'changedBy'];
...
List.prototype.relationship = function(def) {
	...
	if (!utils.isObject(def) || !def.ref || !def.refPath) {
		throw new Error('List Relationships must be specified with an object containing ref and refPath (' + this.key + ')');
	}
	
	if (!def.path) {
		def.path = utils.keyToPath(def.ref);
	}
...
List.prototype.register = function() {
	...
	if (this.get('autokey')) {
		...
		this.schema.pre('save', function(next) {
			...
			if (this.isModified(autokey.from)) {
				this.set(autokey.path, utils.slug(this.get(autokey.from)));
			} else if (!this.get(autokey.path)) {
				this.set(autokey.path, this.id);
			}
	...
	if (_.keys(this.relationships).length) {
		this.schema.methods.getRelated = function(rel, callback) {
			
			var relationship = list.relationships[rel];
			if (!relationship) throw new Error('List.getRelated: list ' + list.key + ' does not have a relationship ' + rel + '.');
			...
			if (!relField || relField.type != 'relationship') throw new Error('List.getRelated: relationship ' + relationship.ref + ' refers to a path (' + relationship.refPath + ') which is not a relationship field.');
