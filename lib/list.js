...
List.prototype.processFilters = function(q) {
	...
	q.split(';').forEach(function(filter) {
		...
		if (ops.field && ops.field.processFilters) {
			ops.field.processFilters(ops, filter);
		} else {
			ops.value = filter[0];
		}
...
List.prototype.getSearchFilters = function(search, add) {
	...
	if (add) {
		_.each(add, function(filter) {
			...
			if (filter.field && filter.field.getSearchFilters) {
				filter.field.getSearchFilters(filter, filters);
			}
		});
	}
