	...
	q.split(';').forEach(function(filter) {
		...
		if (ops.field) {
			switch (ops.field.type) {
				...
				case 'date':
				case 'datetime':
					if (filter[0] === 'bt' || filter[0] === 'gt' || filter[0] === 'lt') {
						ops.operator = filter.shift();
					}
...
List.prototype.getSearchFilters = function(search, add) {
	...
	if (add) {
		_.each(add, function(filter) {
			var cond, path = filter.field.path, value = filter.value;
				...
				switch (filter.field.type) {
					...
					case 'relationship':
						if (value) {
							if (filter.field.many) {
								filters[path] = (filter.inv) ? { $nin: [value] } : { $in: [value] };
							} else {
								filters[path] = (filter.inv) ? { $ne: value } : value;
							}
						} else {
							if (filter.field.many) {
								filters[path] = (filter.inv) ? { $not: { $size: 0 } } : { $size: 0 };
							} else {
								filters[path] = (filter.inv) ? { $ne: null } : null;
							}
						}
					...
					case 'select':
						if (filter.value) {
							...
							filters[path] = (filter.inv) ? { $nin: ['', null] } : { $in: ['', null] };
						}
						break;
					...
					case 'key':
						if (filter.exact) {
							if (value) {
								...
								filters[path] = filter.inv ? { $not: cond } : cond;
							} else {
								if (filter.inv) {
