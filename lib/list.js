...
/**
 * Gets the path of an item based on the options 'preview' key
 *
 * @param {Object} item model to calculate the path for
 * @api private
 */
List.prototype.getPreviewPath = function (item) {
	if(typeof this.options.preview === 'string') {
		return this.options.preview.replace(':key', item[this.autokey.path]);
	}
	if(typeof this.options.preview === 'function') {
		return this.options.preview.call(item, item, this);
	}
	return null;
}
