...
function retryFilename (attempt, file, options, adapter, callback) {
	if (attempt > options.retryAttempts) {
		return callback(Error('Unique filename could not be generated; Maximum attempts exceeded'));
	}
	options.generateFilename(file, attempt, function (err, filename) {
		if (err) return callback(err);
		adapter.fileExists(filename, function (err, exists) {
			if (err) return callback(err);
			if (exists) return retryFilename(attempt + 1, file, options, adapter, callback);
			callback(null, filename);
		});
	});
}

Storage.prototype.getFilename = function (file, callback) {
	...
	switch (options.whenExists) {
		...
		case 'error':
			...
			break;
		case 'retry':
			retryFilename(0, file, options, adapter, callback);
			break;
	}
};
