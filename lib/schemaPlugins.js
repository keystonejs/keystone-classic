...
methods.getRelated = function(paths, callback, nocollapse) {
	...
	_.each(paths, function(options) {
		...
		if (!relationship) throw new Error('List.getRelated: list ' + list.key + ' does not have a relationship ' + relationship + '.');
		...
		queue[relationship.path] = function(done) {
			...
			if (options.related.length) {
				query.exec(function(err, results) {
					...
					async.parallel(_.map(results, function(item) {
	...
	async.parallel(queue, function(err, results) {
		if (!nocollapse && results && rel.length == 1) {
			results = results[rel[0]];
		}
