...
module.exports = function Path (str) {
	...
	this.flatten = function (titlecase) {
		return utils.camelcase(parts.join('_'), titlecase);
	};

	this.flattenplural = function (titlecase) {
		return utils.camelcase([].concat(exceptLast).concat(utils.plural(last)).join('_'), titlecase);
	};

	this.flattensingular = function (titlecase) {
		return utils.camelcase([].concat(exceptLast).concat(utils.singular(last)).join('_'), titlecase);
	};
