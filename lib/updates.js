...
exports.apply = function(callback) {
	...
	if (!fs.existsSync(process.cwd() + '/updates')) {
		throw new Error("Prospekt.applyUpdates() requires an updates folder to exist in the current working directory.");
	}
	
	var updates = _.map(fs.readdirSync(process.cwd() + '/updates'), function(i) { return path.basename(i, '.js'); }).sort(semver.compare);
