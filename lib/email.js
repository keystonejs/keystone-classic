...
Email.prototype.render = function(locals, callback) {
	...
	this.compileTemplate(function(err) {
		...
		// TODO: Need a way to handle live / debug URLs for images...
		// var imagePath = (process.env.NODE_ENV == 'production') ? 'http://www.team9.com.au/img/' : 'http://team9:3000/img/';
		// self.html = self.html.replace(/\/img\//gi, imagePath);
...
Email.prototype.compileTemplate = function(callback) {
	...
	if (templateCache[this.templatePath])
		return callback();
...
Email.prototype.send = function(locals, options, callback) {
	...
	this.render(locals, function(err, email) {
		...
		options.to = ('array' == typeof options.to) ? options.to : [options.to];
		for (var i = 0; i < options.to.length; i++) {
			...
			mergeVars.push({ rcpt: recipient.email, vars: [{ name: 'email', content: recipient.email }] });
		}
