...
if (!_.eq(keystone.get('email transport'), 'mailgun')) {
...
function buildAddress (email, name) {
	if (Array.isArray(email)) {
		return email.map((to) => to.name + ' <' + to.email + '>').join(',');
	} else {
...
var templateCSSMethods = {
	shadeColor: function (color, percent) {
		...
		var num = parseInt(color.slice(1), 16), amt = Math.round(2.55 * percent), R = (num >> 16) + amt, G = (num >> 8 & 0x00FF) + amt, B = (num & 0x0000FF) + amt;// eslint-disable-line one-var
