...
var Email = function (options) {
	 ...
	 *
	 * NOTE: Enable these in 4.1 or 4.2 release. These patterns will be deprecated with the 0.5 release!
	 */
	// var stackTrace;
	// // templateExt -> engine
	// if (options.templateExt) {
	// 	if (!options.engine) {
	// 		stackTrace = new Error('[Keystone.Email] You\'re using the old "templateExt" option, please set the "engine" instead!').stack;
	// 		console.log(stackTrace);
	// 		options.engine = options.templateExt;
	// 	} else {
	// 		stackTrace = new Error('[Keystone.Email] You\'re using the old "templateExt" option in combination with the new "engine" option. Please remove the deprecated "templateExt" option and only use "engine" option!').stack;
	// 		console.log(stackTrace);
	// 	}
	// }
	...
	// if (rootPath) {
	// 	if (!options.root) {
	// 		stackTrace = new Error('[Keystone.Email] You\'re using the old "keystone.set(\'emails\')" option, please use the "root" option instead!').stack;
	// 		console.log(stackTrace);
	// 		options.root = rootPath;
	// 	} else {
	// 		stackTrace = new Error('[Keystone.Email] You\'re using the old "keystone.set(\'emails\')" option in combination with the new "root" option. Please remove the deprecated "keystone.set(\'emails\')" option and only use the "root" option!').stack;
	// 		console.log(stackTrace);
	// 	}
	// }
	...
	return new EmailInstance(options.templateName, options);
};
