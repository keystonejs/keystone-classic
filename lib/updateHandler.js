...
UpdateHandler.prototype.process = function (data, options, callback) {
	...
	async.forEach(options.fields, function (path, callback) {
		...
		if (!invalidated && !field.inputIsValid(data)) {
			if (field.options.invalidMessage && Array.isArray(field.options.invalidMessage)) {
				field.options.invalidMessage.forEach(function (im) {
					addValidationError(field.path, im);
				});
			} else {
