	...
	options.fields.forEach(function(path) {
		...
		switch (field.type) {
			...
			case 'location':
				actionQueue.push(field.getRequestHandler(this.item, this.req, function(err) {
					if (err && options.flashErrors) {
						req.flash('error', field.label + ' improve failed - ' + (err.status_text || err.status));
					}
					progress(err);
				}));
			break;
			...
			case 'password':
				...
				if (!data[field.path] && (!field.required || item.get(field.path)))
					return;
		...
		if (!invalidated && !field.validateInput(data)) {
			addValidationError(field.path, 'Please enter a valid ' + field.typeDescription + ' in the ' + field.label + ' field.');
			invalidated = true;
		}
		
		// validate required fields, unless they've already been invalidated by field-specific behaviour
		if (!invalidated && field.required && !field.validateInput(data, true, this.item)) {
