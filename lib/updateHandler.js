...
UpdateHandler.prototype.process = function(data, options, callback) {
	...
	options.fields.forEach(function(path) {
		...
		switch (field.type) {
			...
			case 'password':
				...
				if (data[field.path] != data[field.paths.confirm]) {
					var message = options.invalidMessages[field.path + '_match'] || 'Passwords must match';
		...
		if (!invalidated && !field.validateInput(data)) {
			...
			var message = options.invalidMessages[field.path] || field.options.invalidMessage || 'Please enter a valid ' + field.typeDescription + ' in the ' + field.label + ' field';
			addValidationError(field.path, message);
			invalidated = true;
		}
		...
		if (!invalidated && options.required[field.path] && !field.validateInput(data, true, this.item)) {
			...
			var message = options.requiredMessages[field.path] || field.options.requiredMessage || field.label + ' is required';
			addValidationError(field.path, message);
			invalidated = true;
		}
