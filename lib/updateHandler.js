	...
	options.fields.forEach(function(path) {
		...
		switch (field.type) {
			...
			case 'password':
				...
				if (!data[field.path] && (!field.required || this.item.get(field.path)))
					return;
		...
		if (!invalidated && !field.validateInput(data)) {
			var message = field.options.invalidMessage || 'Please enter a valid ' + field.typeDescription + ' in the ' + field.label + ' field.';
		...
		if (!invalidated && field.required && !field.validateInput(data, true, this.item)) {
			var message = field.options.requiredMessage || field.label + ' is required.';
			addValidationError(field.path, message);
			invalidated = true;
		}
		...
		if (!invalidated && this.validationMethods[field.path]) {
			var message = this.validationMethods[field.path](data);
			if (message) {
				addValidationError(field.path, message);
			}
		}
