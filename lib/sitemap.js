				...
				if (results && results.length > 0) {
					results.forEach(function (v, i) {
						...
						if (list.publishing && list.publishing.publishStates && v.publishing && v.publishing.state) {
							console.log('Sitemap:' + v.name + ' Publishing ENABLED: States:' + list.publishing.publishStates);
							
							if (list.publishing.publishStates.indexOf(v.publishing.state , 0) === -1) {
								include = false;
								
							}
						} else {
							console.log('Sitemap:' + v.name + ' Publishing NOT ENABLED States:');
						}
