 ...
 *      stateField: 'state', // field to publishing content state
...
module.exports = function publishing () {
    ...
    // Enabled only if 'publishing' option is enabled for a list
	...
	list.add('Publishing', {
		publishing: {
            ...
            approvalPending: { type: Boolean, hidden: true, default: false },
			approveRequest: { type: Boolean, hidden: true, default: false },
			schedulePublish: { label: 'Schedule Publish', type: Boolean, default: false },
			publishDate: { label: 'Publish Date', type: Types.Datetime, index: true, dependsOn: { ['publishing.schedulePublish']: true } },
			scheduleUnpublish: { label: 'Schedule Unpublish', type: Boolean, default: false },
	...
	list.schema.pre('save', function (next) {
		...
		if (this.get('publishing.requestApproval') != this.get(options.stateField)) {
                ...
                if (canUserApprove) {
                    this.set('state', this.get('publishing.requestApproval'));
