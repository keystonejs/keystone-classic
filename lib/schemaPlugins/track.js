	...
	list.schema.pre('save', function (next) {
		...
		if (this.isNew) {
			if (options.createdAt && !this.get(options.createdAt)) {
				this._set(options.createdAt, now);
			}
			if (options.createdBy && this._req_user && !this.get(options.createdBy)) {
				this._set(options.createdBy, this._req_user._id);
			}
		}
		...
		if (this.isNew || this.isModified()) {
			if (options.updatedAt) {
				this._set(options.updatedAt, now);
			}
			if (options.updatedBy && this._req_user) {
				this._set(options.updatedBy, this._req_user._id);
			}
		}
