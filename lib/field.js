...
function Field(list, path, options) {
	...
	this.templateDir = options.template || (__dirname + '../../templates/fields/' + this.type);
...
Field.prototype.compile = function(templateType,callback){
	var templateType = templateType || 'form',
	 	filePath = path.resolve(this.templateDir+'/'+templateType+'.jade');
	if(!fieldTemplates[filePath]){
		fs.readFile(filePath,'utf8',function(err,data){
			if(err)
				return callback();
			fieldTemplates[filePath] = jade.compile(data,{
				filename:filePath,
...
Field.prototype.render = function(templateType,item){
	var templateType = templateType || 'form',
	 	filePath = path.resolve(this.templateDir+'/'+templateType+'.jade');
	if(!fieldTemplates[filePath]){
		fieldTemplates[filePath] = jade.compile(fs.readFileSync(filePath, 'utf8'), {
			filename:filePath,
			pretty: keystone.get('env') != 'production'
		});
	}
	return fieldTemplates[filePath]({
		field:this,
		item:item,
		utils:utils
	})
