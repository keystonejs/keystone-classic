...
Field.prototype.compile = function(type, callback) {
	...
	if (!compiledTemplates[templatePath]) {
		fs.readFile(templatePath, 'utf8', function(err,data) {
			if (!err){
				template = jade.compile(data, { filename:templatePath, pretty:keystone.get('env') != "production" });
				compiledTemplates[templatePath] = template;
			}
...
Field.prototype.render = function(type, item) {
	...
	if (!compiledTemplates[templatePath]) {
		//Compile synchronous if not already compiled
		file = fs.readFileSync(templatePath, 'utf8');
		template = jade.compile(data, { filename:templatePath, pretty:keystone.get('env') != "production" });
		compiledTemplates[templatePath] = template;
	} else {
		template = compiledTemplates[templatePath];
	}

	return template({ field:this, item:item, utils:utils });
