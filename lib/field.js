...
function Field(list, path, options) {
	...
	this.template = options.template || (__dirname + '../../templates/fields/' + this.type);
...
Field.prototype.render = function(type,item){
	type = type || 'form';  
	this.templates = this.templates || {};
	var jadeOptions = {
		filename: this.template+'/'+type+'.jade',
		pretty: keystone.get('env') != 'production'
	};
	var compile = function(opts) {
		return jade.compile(fs.readFileSync(opts.filename, 'utf8'), opts);
	}
	var template = this.templates[type]? this.templates[type] : (this.templates[type] = compile(jadeOptions));

	var html = template({field:this,item:item});
	return html;
}
