			...
			async.each(lists, function(key, doneList) {
				...
				async.each(data[key], function(srcData, doneItem) {
					...
					async.each(relationships, function(field, doneField) {
						...
						if ( !field.path )  {
							writeLog("WARNING:  Invalid relationship (undefined list path) [lList: "+key+"]");
						...
						if ( !field.refList.key )  {
							writeLog("WARNING:  Invalid relationship (undefined ref list key) [list: "+key+"] [field.refList: "+field.refList+"] [fielValue: "+fieldValue+"]");
							stats[list.key].warnings++;
							return doneField();
						}
						else
						...
						if (options.verbose) {
							writeLog('Populated ' + relationshipsUpdated + ' relationship' + (relationshipsUpdated === 1 ? '' : 's') + '.');
						}
