...
function routes(app) {
	...
	if (this.get('auth') === true) {
		...
		app.all('/keystone/signin', require('../../admin/routes/signin'));
		app.all('/keystone/signout', require('../../admin/routes/signout'));
	...
	// Admin UI Home
	app.all('/keystone', require('../../admin/routes/home'));
	...
	app.use('/keystone/api', require('../../admin/api/middleware/apiError'));

	// Generic Lists API
	app.all('/keystone/api/counts', require('../../admin/api/counts'));
	app.get('/keystone/api/:list', initList(), require('../../admin/api/list/get'));
	app.get('/keystone/api/:list/:format(export.csv|export.json)', initList(), require('../../admin/api/list/download'));
	...
	app.post('/keystone/api/:list/:id', initList(), require('../../admin/api/item/update'));
	app.post('/keystone/api/:list/:id/delete', initList(), require('../../admin/api/item/delete'));

	// DEPRECATED app.all('/keystone/api/:list/:action(autocomplete|order|create|fetch)', initList(), require('../../admin/api/list'));
	// DEPRECATED app.all('/keystone/download/:list', initList(), require('../../admin/api/download'));

	// Admin UI List
	app.all('/keystone/:list/:page([0-9]{1,5})?', initList(true), require('../../admin/routes/list'));
	app.all('/keystone/:list/:item', initList(true), require('../../admin/routes/item'));
