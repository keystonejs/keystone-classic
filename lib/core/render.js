...
function render(req, res, view, ext) {
	...
	// }
	/*
		revise some locals var before pass in template
	 */
	var lang = req.query.lang ? req.query.lang : '';
	var nav = keystone.nav;
	var uploadedRootFolder = keystone.get('uploadedRootFolder') ? keystone.get('uploadedRootFolder') : '';
	/*
	_.each(nav, function (value, key) {
		var query = lang ? '?lang=' + lang : '';
		if (_.isArray(value)) {
			_.each(value, function (item, subkey) {
				nav[key][subkey] = item + query;
			});
...
*/
	...
	var locals = {
		...
		lang: lang,
		uploadedRootFolder: uploadedRootFolder
	};
