...
var url = require('url');
var metrics = require('keystone-metrics');
 ...
 *
 * Example:
 *     keystone._set('user model', 'User') // sets the 'user model' option to `User`
 */
exports.set = function (key, value) {
	if (arguments.length === 1) {
		return this._options[key];
	} else {
		var sanitizedValue = metrics.sanitize(key, value);
		metrics.setOption(key, sanitizedValue);
		this._set(key, value);
	}
};

exports._set = function (key, value) {
	switch (key) {
		...
		case 'module root':
			...
			if (!isAbsolutePath(value)) {
				var callerData = caller(2);
				value = path.resolve(path.dirname(callerData), value);
			}
			break;
		case 'app':
			this.app = value;
