...
function mount(mountPath, parentApp, events) {
  debug('mounting');
	...
	// unless the headless option is set (which disables the Admin UI),
	// include browserify-middleware for the Admin UI scripts
	if (!this.get('headless')) {
		debug('adding keystone admin ui browserify-middleware');
		var external = require('../../admin/packages');
		var babelify = require('babelify');
		// views are transpiled on demand
		app.use('/keystone/js', browserify(path.join(__dirname, '..', '..', 'admin', 'src', 'views'), {
			external: external,
			transform: [babelify]
		}));
	}
	...
	debug('adding standard middleware');
