...
localfiles.prototype.uploadFiles = function(item, files, update, callback) {
	...
	async.map(files, function(file, processedFile) {
		...
		if (field.options.allowedTypes && !_.contains(field.options.allowedTypes, file.type)){
			return processedFile(new Error('Unsupported File Type: ' + file.type));
		}
		...
		var doMove = function(doneMove) {
			...
			fs.move(file.path, path.join(field.options.dest, name), { clobber: field.options.overwrite }, function(err) {
