...
function embedly(list, path, options) {
	...
	if (!keystone.get('embedly api key')) {
		throw new Error('Embedly fields require the embedly api key option to be set.');
	}
...
embedly.prototype.addToSchema = function() {
	...
	schema.pre('save', function(next) {
		...
		embedly.expand(this.get(field.fromPath), function(err, embed) {
			
			if (embed) {
				post.videoEmbedData.exists = true;
				post.videoEmbedData.authorName = embed.author_name;
				post.videoEmbedData.authorUrl = embed.author_url;
				post.videoEmbedData.description = embed.description;
				post.videoEmbedData.height = embed.height;
				post.videoEmbedData.html = embed.html;
				post.videoEmbedData.providerName = embed.provider_name;
				post.videoEmbedData.providerUrl = embed.provider_url;
				post.videoEmbedData.thumbnailHeight = embed.thumbnail_height;
				post.videoEmbedData.thumbnailUrl = embed.thumbnail_url;
				post.videoEmbedData.thumbnailWidth = embed.thumbnail_width;
				post.videoEmbedData.title = embed.title;
				post.videoEmbedData.type = embed.type;
				post.videoEmbedData.url = embed.url;
				post.videoEmbedData.version = embed.version;
				post.videoEmbedData.width = embed.width;
			} else {
				post.videoEmbedData.exists = false;
			}
