...
cloudinaryimages.prototype.getRequestHandler = function(item, req, callback) {
	...
	return function() {
		...
		// Order
		if (req.body[field.paths.order]) {
			var images = item.get(field.path),
				newOrder = req.body[field.paths.order].split(',');
		...
		if (req.body[field.paths.uploads]) {
			var uploads = JSON.parse(req.body[field.paths.uploads]);
