...
cloudinaryimages.prototype.addToSchema = function() {
	...
	field.underscoreMethod('delete', function(i) {
		var image = this.get(field.path)[i];
		if (image) {
			cloudinary.uploader.destroy(image.public_id, function() {});
			this.get(field.path).splice(i,1);
			this.save();
		}
	});
}
	...
	return function() {
		...
		if (req.body) {
			...
			_.each(actions, function(act) {
				...
				var r = act[0];
				act.splice(0,1);
				if (!r.match(/remove|delete/)) return;
				_.each(act, function(d) {
					if (r == 'delete') {
						cloudinary.uploader.destroy(d, function() {});
					}
					_.find(item.get(field.path), function(v,i) {
						if (v.public_id == d)
							item.get(field.path).splice(i,1);
					});
				});
			});
			
			// TW 2013-10-02: Review
		...
		if (req.files && req.files[field.paths.upload]) {
			...
			var files = !_.isArray(req.files[field.paths.upload]) ? [req.files[field.paths.upload]] : req.files[field.paths.upload];
