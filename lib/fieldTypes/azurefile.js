...
function azurefile(list, path, options) {
	...
	if (!this.azurefileconfig) {
		throw new Error('Invalid Configuration\n\n' +
			...
			'See http://keystonejs.com/docs/configuration#azurefile for more information.\n');
	}
	...
	self.azurefileconfig.container = self.azurefileconfig.container || 'keystone';
...
azurefile.prototype.pre = function(event, fn) {
	if (!this._pre[event]) {
		throw new Error('azurefile (' + this.list.key + '.' + this.path + ') error: azurefilefield.pre()\n\n' +
