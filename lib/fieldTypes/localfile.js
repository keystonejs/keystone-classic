...
function localfile(list, path, options) {
	...
	this._pre = {
		...
		'delete': [] // Before the file is deleted
	};

	this._post = {
		move: [], // After file is moved into final destination
		'delete': [] // After the file is deleted
	};
	...
	if (options.pre) {
		...
		if (options.pre['delete']) {
			this._pre['delete'] = this._pre['delete'].concat(options.pre['delete']);
		}
	}

	if (options.post) {
		if (options.post.move) {
			this._post.move = this._post.move.concat(options.post.move);
		}
		if (options.post['delete']) {
			this._post['delete'] = this._post['delete'].concat(options.post['delete']);
		}
	}
}
	...
	var schemaMethods = {
		...
		'delete': function() {
			...
			var doDelete = function(item) {
				if (exists(item)) {
					fs.unlinkSync(path.join(item.get(paths.path), item.get(paths.filename)));
				}
				reset(item);
			};

			async.eachSeries(field._pre['delete'], function(fn, next) {
				fn(item, item.get(field.path), next);
			}, function() {

				var itemCopy = util._extend({}, item);
				var fileCopy = util._extend({}, item.get(field.path));
				doDelete(item);

				async.eachSeries(field._post['delete'], function(fn, next) {
					fn(itemCopy, fileCopy, next);
				});
...
localfile.prototype.format = function(item) {
	if (this.hasFormatter())
		return this.options.format(item, item.get(this.path));
