...
fsimage.prototype.addToSchema = function() {
    ...
    var uploaderMethods = {
        ...
        imAction: function(action, file, prefix, size, callback) {
            ...
            var filePath = this.uploadDir + prefix;

            if (action == 'resize'){
                im.resize(this.imOptions(file, filePath, size), function(err, stdout, stderr){
                    return callback(err);
                });
            }
            else {
                im.crop(this.imOptions(file, filePath, size), function(err, stdout, stderr){
                    return callback(err);
                });
            }
        },
        ...
        imOptions: function(file, targetFile, size) {
