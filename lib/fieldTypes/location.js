...
function location(list, path, options) {
	this._underscoreMethods = ['format', 'list', 'googleLookup', 'kmFrom', 'milesFrom'];
	...
	if (options.required) {
		if (Array.isArray(options.required)) {
			...
			this.requiredPaths = options.required.split(' ');
		} else {
...
location.prototype.addToSchema = function() {
	...
	schema.pre('save', function(next) {
		...
		if (Array.isArray(obj.geo) && 0 === obj.geo.length) {
			obj.geo = undefined;
		}
		next();
	});
...
/**
 * Outputs a list of the values of specified sub-paths. Only paths that
 * have values will be included.
 * 
 * Sub-paths should be specified as a space-separated list.
 * 
...
location.prototype.list = function(item, values, delimiter) {
	
	var paths = this.paths;
	
	values = paths.split(' ').map(function(i) {
...
location.prototype.validateInput = function(data, required) {
	if (required) {
		var paths = this.paths,
			valid = true;
...
location.prototype.getRequestHandler = function(item, req, callback) {
	...
	return function() {
		...
		var update = req.body[field.paths.overwrite] ? 'overwrite' : true;
		
		if (req.body && req.body[field.paths.improve]) {
...
location.prototype.handleRequest = function(item, req, callback) {
