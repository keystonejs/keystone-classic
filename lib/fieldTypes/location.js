...
location.prototype.validateInput = function(data, required, item) {
	...
	var paths = this.paths,
		nested = _.isObject(data[this.path]),
		values = nested ? data[this.path] : data,
		valid = true;
...
location.prototype.updateItem = function(item, data) {
	...
	if (_.isObject(data[this.path])) {
		...
		values = _.pick(data[this.path], valueKeys);
	} else {
		// Handle flattened values
		valueKeys = valueKeys.map(function(i) {
			return paths[i];
		});
		values = _.pick(data, valueKeys);
	}

    var setValue = function(key) {
        if (values.hasOwnProperty(paths[key]) && values[paths[key]] !== item.get(key)) {
            item.set(paths[key], values[paths[key]] || null);
        }
    };

	_.each(fieldKeys, setValue);

	if (paths.geo in values) {

		var oldGeo = item.get(paths.geo) || [],
			newGeo = values[paths.geo];
	...
	} else if (paths.geo_lat in values && paths.geo_lng in values) {

		var lat = utils.number(values[paths.geo_lat]),
			lng = utils.number(values[paths.geo_lng]);
