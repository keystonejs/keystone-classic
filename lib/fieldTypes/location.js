...
location.prototype.addToSchema = function() {
	...
	schema.virtual(paths.serialised).get(function() {
		return _.compact([
			this.get(options.path + '.number'),
			this.get(options.path + '.name'),
			this.get(options.path + '.street1'),
			this.get(options.path + '.street2'),
			this.get(options.path + '.suburb'),
			this.get(options.path + '.state'),
			this.get(options.path + '.postcode'),
			this.get(options.path + '.country')
		]).join(', ');
	});
...
location.prototype.updateItem = function(item, data) {
	...
	var setValue = function(key) {
		console.log('setting data for ' + paths[key] + ' to ' + data[paths[key]]);
